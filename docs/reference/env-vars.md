...
| `OPENTERFACE_WINDOW_MAX_SIZE` | _(unset)_ | `WIDTHxHEIGHT`, each dimension `1`–`32767` | Same as `connect --window-max-size`: optionally caps the video/content window area in physical pixels. When unset, the compositor may make the window larger and the renderer scales/letterboxes the current capture. |
| `OPENTERFACE_CAPTURE_SIZING` | `adaptive` | `adaptive`/`fixed` | `adaptive` reconfigures capture to the best supported mode for the displayed physical window size, capped by the device's maximum mode. `fixed` keeps the default capture request for the whole session. |
