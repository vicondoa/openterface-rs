...
**Focus.** Operator UX and completeness of the user-facing surface.

**Looks for.**
- CLI commands/flags cover the supported surface (`connect` + flags, `scan`,
  `status`, `reset`, `--version`, `-v`) and the env-var surface
  (`OPENTERFACE_*`).
...
complete, with no actionable `recommendations`.
