...
pub enum InputEvent {
    ...
    /// A key press or release generated by a paste operation.
    ///
    /// Paste keys are encoded like regular keys, but the scheduler can discard
    /// queued paste frames on [`InputEvent::ReleaseAll`] so focus loss, close, or
    /// abort does not have to drain a long paste backlog before releasing input.
    PasteKey {
        /// The HID usage of the key.
        usage: HidUsage,
        /// Modifiers held with this key.
    ...
    pub fn is_release(self) -> bool {
        matches!(
            ...
            InputEvent::Key { pressed: false, .. }
                | InputEvent::PasteKey { pressed: false, .. }
...
mod tests {
    ...
    fn release_detection() {
        ...
        let paste_release = InputEvent::PasteKey {
            usage: HidUsage(0x04),
            modifiers: Modifiers::NONE,
            pressed: false,
        };
        ...
        assert!(paste_release.is_release());
    }
