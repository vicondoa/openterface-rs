        ...
        V: VideoSource + 'static,
    {
        ...
        let writer = {
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || writer_loop(&mut serial, &input_rx, pacing, &running))
...
fn writer_loop<T: SerialTransport>(
    ...
    while running.load(Ordering::SeqCst) {
        ...
        match input_rx.recv_timeout(wait) {
            Ok(event) => scheduler.submit(event),
    ...
    // Final drain so a trailing release is not lost on a clean shutdown.
    while let Some(cmd) = scheduler.poll(Instant::now()) {
        if serial.write_all(&cmd).is_err() {
            return;
        }
...
mod tests {
    ...
    fn ctrl_alt_del_reaches_serial_then_shuts_down_cleanly() {
        ...
        // The keyboard press (mods 0x05, key 0x4C) must reach the serial line.
        let got = wait_until(
            || {
                let buf = sink.lock().unwrap();
                buf.windows(2).any(|w| w == [0x05, 0x00]) && buf.contains(&0x4C)
            },
            Duration::from_secs(2),
        );
        assert!(got, "Ctrl+Alt+Del frame did not reach the serial transport");
